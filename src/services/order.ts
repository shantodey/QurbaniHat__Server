...
router.patch("/:id", async (req, res) => {
    try {
        ...
        if (payload.userphone) payload.userphone = Number(payload.userphone);
        if (payload.orderprice) payload.orderprice = Number(payload.orderprice);
        if (payload.orderweight) payload.orderweight = Number(payload.orderweight);
